...
##        self.model = None
##        self.dt = None
##        self.features ={'Time','DayPart','DayType'}
##        self.target = 'Items'
...
##        pay = pd.DataFrame(payload, index=[0])
##        pay['DayPart'] = self.pay['DayPart'].apply(lambda x: 1 if x == 'Morning' else 0)
##        pay['DayType'] = self.pay['DayType'].apply(lambda x: 1 if x == 'Weekend' else 0)
##        item = np.squeeze(self.model.predict_proba(pay))
##        return {'item':item}
